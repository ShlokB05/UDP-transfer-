...
def send_reliable(cs, filedata, receiver_binding, win_size):
    ...
    def transmit_entire_window_from(left_edge):
        ...
        while latest_tx < win_right_edge:
            ...
            latest_tx = latest_tx + len(msg)
        return latest_tx

            # TODO: # After transmitting each packet
            # use `cs.sendto(...)` with a properly constructed
            # Msg object and print the same "Transmitted ..."
            # message as used below in `transmit_one()`.
            #
            # The function MUST return the sequence number that
            # follows the last byte transmitted.
            #
    ...
    Lastackd = transmit_entire_window_from(win_left_edge)
    # TODO: This is where you will make the rest of your changes.
    while win_left_edge < INIT_SEQNO + content_len:
        Read,Write,Exception = select.select([cs],[],[],RTO)
        if(len(Read) != 0): #if it executed:
            ...
            ackNum = ack_msg.ack
            if ackNum <= win_left_edge: 
                continue
            win_left_edge = ackNum
            if(win_left_edge >= (INIT_SEQNO+content_len)):  #reached end
                break
            if((win_left_edge + win_size) < (INIT_SEQNO + content_len)):
                win_right_edge = win_left_edge + win_size
            else:
                win_right_edge = INIT_SEQNO + content_len
                
            if(Lastackd < win_right_edge):
                Lastackd = transmit_entire_window_from(Lastackd)
        else:
            transmit_one()           
